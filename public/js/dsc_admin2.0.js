$(function(){
    ...
    $('.letter').find('a[data-letter]').click(function(){
		...
		$.jqueryAjax('http://dsc.com/admin/js/get_ajax_content.php', 'act=search_brand_list&goods_id='+goods_id+'&letter='+letter, function(data){			
			if(data.content){
    ...
    $('.b_search').find('a').click(function(){
		...
		$.jqueryAjax('http://dsc.com/admin/js/get_ajax_content.php', 'act=search_brand_list&goods_id='+goods_id+'&keyword='+keyword, function(data){
			if(data.content){				
				$(".brand-list").html(data.content);
				$(".brand-not").hide();
			}else{
	...
	$(document).on("click","a[ectype=sub]",function(){
		...
		if(dealType == 0){
			if(label == 0){
				$.jqueryAjax("http://dsc.com/admin/js/get_ajax_content.php", "act="+operation+"&goods_id="+goods_id+"&value="+value+extension, function(data){
				...
				$.jqueryAjax("http://dsc.com/admin/js/get_ajax_content.php", "act="+operation+"&sec_id="+sec_id+"&tb_id="+tb_id+"&value="+value+extension, function(data){
					obj.parents(".step[ectype=filter]").find(".move_right .move_list").html(data.content);
	...
	$("a[ectype='search']").on("click",function(){
		...
		function ajax(){
			$.jqueryAjax('http://dsc.com/admin/js/get_ajax_content.php', 'act=filter_list&search_type='+search_type+search_data, function(data){
...
jQuery.category = function(){
	...
	$(document).on('click', '.select-list li', function(){
		...
		$.jqueryAjax('http://dsc.com/admin/js/get_ajax_content.php', 'act=filter_category&cat_id='+cat_id+"&cat_type_show=" + cat_type_show + "&user_id=" + user_id + "&table=" + table, function(data){
			if(data.content){
...
//        var _this = $("[ectype='"+focus+"']");
        ...
        var url = (actionUrl) ? actionUrl : 'http://dsc.com/admin/js/get_ajax_content.php';
        page = parseInt(page);
        if(page){
            //下一页
            if(type == 'next'){
